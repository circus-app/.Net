...
namespace Circus.Wpf.Data {
    ...
    [Obsolete("Use Circus.Model.PropertyChangedManager instead.")]
    public sealed class PropertyChangedManager {
        private PropertyChangedManager() { 
        }
        /// <summary>Adds the DataSource.OnItemPropertyChanged event handler for properties of the provided object defined in its PropertyChangeAttribute.</summary>
        public static void AddHandler(DataSource data, object obj) {
            if (PropertyChangedManager.IsPropertyChanged(obj, out PropertyChangedAttribute attribute, out INotifyPropertyChanged notify)) {
                foreach (string name in attribute.Values) {
                    PropertyChangedEventManager.AddHandler(notify, data.OnItemPropertyChanged, name);
                }
            }
        }
        private static bool IsPropertyChanged(object obj, out PropertyChangedAttribute attribute, out INotifyPropertyChanged notify) {
            attribute = obj.GetType().GetCustomAttribute<PropertyChangedAttribute>();
            notify = Assert.Is(obj, out INotifyPropertyChanged changed) ? changed : null;
            return Assert.NotNull(attribute) && Assert.NotNull(notify);
        }
        /// <summary>Removes the DataSource.OnItemPropertyChanged event handler for properties of the provided object defined in its PropertyChangeAttribute.</summary>
        public static void RemoveHandler(DataSource data, object obj) {
            if (PropertyChangedManager.IsPropertyChanged(obj, out PropertyChangedAttribute attribute, out INotifyPropertyChanged notify)) {
                foreach (string name in attribute.Values) {
                    PropertyChangedEventManager.RemoveHandler(notify, data.OnItemPropertyChanged, name);
                }
            }
        }
    }
